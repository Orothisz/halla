...
/* ---------- Dress Code pill (Hero) ---------- */
function DressCodePill() {
  if (!ITINERARY?.length) return null;
  const day1 = ITINERARY.find((d) => d.day === 1);
  const day2 = ITINERARY.find((d) => d.day === 2);
  return (
    <div className="mt-3 flex items-center justify-center gap-2 flex-wrap">
      <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs border border-white/20">
        <Shirt size={14} /> Day 1: {day1?.dressCode || "—"}
      </span>
      <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs border border-white/20">
        <Shirt size={14} /> Day 2: {day2?.dressCode || "—"}
      </span>
    </div>
  );
}
...
/* ---------- Itinerary Cards ---------- */
function DayCard({ day, dateText, dressCode, events }) {
  return (
    <div className="rounded-2xl border border-white/12 bg-white/[0.04] p-4 md:p-6 backdrop-blur-sm relative overflow-hidden">
      <div
        aria-hidden
        className="absolute -top-10 -right-10 h-24 w-24 rounded-full bg-white/10 blur-2xl opacity-40"
      />
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold">Day {day} • {dateText}</div>
            <div className="text-xs text-white/70">Dress Code: {dressCode}</div>
          </div>
        </div>
        <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs border border-white/20">
          <Shirt size={14} /> {dressCode}
        </span>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-2">
        {events.map((e, idx) => (
          ...
          >
            ...
            <div className="text-xs text-white/60">{e.time}</div>
          </div>
        ))}
      </div>
    </div>
...
function ItinerarySection() {
  ...
  return (
    <section className="mt-16 rounded-[28px] border border-white/12 p-6 md:p-10 bg-white/[0.04] backdrop-blur-sm ring-1 ring-white/5">
      <SectionHeading
        kicker="Chapter II½"
        title="Itinerary & Dress Code"
        icon={<Calendar size={20} className="text-white/70" />}
      />
      <p className="text-white/80 leading-relaxed">
        Plan your days at Noir — timings are tight, decorum is gold, and style is on theme.
      </p>
      ...
      <div className="mt-4 text-xs text-white/60">
        * Staggered lunch timings will be informed by committee staff on the day.
      </div>
    </section>
  );
}
...
function PartnersSection() {
  ...
  return (
    <section className="mt-16 rounded-[28px] border border-white/12 p-6 md:p-10 bg-white/[0.04] backdrop-blur-sm ring-1 ring-white/5">
      ...
      />
      <p className="text-white/80 leading-relaxed">
        Institutions and brands that stand with Noir — strengthening access, study, and community.
      </p>
