...
/* ----------- File → base64 (handles non-File values safely) ----------- */
...
/* ----------- Roman Layer (match Home, tuned for Register) ----------- */
function RomanLayer() {
...
export default function Register() {
  ...
  useEffect(() => {
    if (busy) {
      ...
      return () => { clearTimeout(t1); clearTimeout(t2); window.onbeforeunload = null; };
    }
  }, [busy]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        // clear file preview only; keep text fields so they can duplicate-register faster
    ...
    <div className="min-h-screen text-white relative"
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        <NoirCard className="p-6 md:p-8">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight"><Gilded>Registration</Gilded></h1>
              <p className="mt-2 text-white/80">Step in with clarity. Your details are autosaved; sign in to sync across devices.</p>
              <div className="mt-4 flex flex-wrap gap-3">
                <a href={MATRIX_HREF} target="_blank" rel="noreferrer"
          ...
          <div className="mt-6 grid md:grid-cols-2 gap-6">
            <div className="space-y-5">
              <Field label="Committee Preference 1" required error={err.committeePref1}>
                <SelectMenu value={f.committeePref1} onChange={e=>update({committeePref1:e.target.value})} options={committees}/>
              </Field>
            ...
            <div className="space-y-5">
              <Field label="Committee Preference 2" required error={err.committeePref2}>
                <SelectMenu value={f.committeePref2} onChange={e=>update({committeePref2:e.target.value})} options={committees}/>
              </Field>
        ...
        <NoirCard className="p-6 md:p-8 mt-8">
          ...
          <p className="mt-2 text-sm text-white/80">Early Bird: <b>₹2,000</b> per delegate (IPL: <b>₹2,000</b>). Payment & proof are required.</p>
          ...
          <div className="grid lg:grid-cols-[1fr_300px] gap-6 mt-4">
            <div className="space-y-4">
              ...
              <div className="flex gap-2 flex-wrap">
                <button onClick={()=>copyUPI(UPI_PRIMARY)} className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/10 hover:bg-white/20 px-3 py-2 text-sm">
                  {copied ? <Check size={14}/> : <Copy size={14}/> } Copy {UPI_PRIMARY}
                </button>
                <button onClick={()=>copyUPI(UPI_ALT)} className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/10 hover:bg-white/20 px-3 py-2 text-sm">
              ...
              <div className="rounded-xl border border-white/15 bg-white/5 p-2 grid place-items-center">
                {f.paymentPreview
                  ? <img src={f.paymentPreview} alt="Payment preview" className="max-h-44 rounded-lg object-contain"/>
        ...
        <NoirCard className="p-6 md:p-8 mt-8 mb-24">
          ...
          <h2 className="mt-1 text-2xl md:text-3xl font-extrabold"><Gilded>Final Step</Gilded></h2>
      ...
      <AnimatePresence>
        {busy && (
          <motion.div className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm grid place-items-center"
