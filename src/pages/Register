...
/* ---------------------------------------------
 * BACKEND ENDPOINT (your Apps Script Web App)
 * --------------------------------------------- */
const REGISTER_API_URL =
  "https://script.google.com/macros/s/AKfycbx7maPoZf_YhaXuI18SMEgPC9YAEUnOGHRTxEky-cij-1f-kktAFBWIsCipyO9_KsAXKw/exec";
// If your Apps Script requires X-API-KEY and you set it to the script ID, this matches:
const REGISTER_API_KEY =
  "AKfycbx7maPoZf_YhaXuI18SMEgPC9YAEUnOGHRTxEky-cij-1f-kktAFBWIsCipyO9_KsAXKw";
...
/* ---------- Supabase auth + cloud draft sync ---------- */
...
export default function Register() {
  ...
  // On login, hydrate from cloud draft (if newer than local)
  useEffect(() => {
    (async () => {
      if (!loggedIn) return;
      const cloud = await loadCloudDraft(user.id);
      if (cloud) {
        // merge conservatively: prefer local if it has email/whatsapp and cloud doesn't
        const local = JSON.parse(localStorage.getItem(DRAFT_KEY) || "null");
        const merged = { ...(local || {}), ...cloud };
        setF(merged);
        localStorage.setItem(DRAFT_KEY, JSON.stringify(merged));
      } else {
        // first login with local draft → upload
        const local = JSON.parse(localStorage.getItem(DRAFT_KEY) || "null");
  ...
  // autosave (debounced) → local + (if logged) cloud
  ...
  const submit = async () => {
    ...
    try {
      const payload = {
        ...
        paymentProofLink: "", // backend fills this after Drive upload
      };
      ...
      // Cloud: preserve draft, or clear payment file only
  ...
  return (
    <div className="min-h-screen text-white relative">
      {/* background glows */}
      <div className="pointer-events-none fixed -top-28 -left-28 w-[28rem] h-[28rem] rounded-full blur-3xl bg-white/10 -z-10" />
      <div className="pointer-events-none fixed -bottom-28 -right-28 w-[32rem] h-[32rem] rounded-full blur-3xl bg-white/10 -z-10" />
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        <NoirCard className="p-6 md:p-10 relative overflow-hidden">
          <div
            className="absolute -z-10 inset-0 opacity-[.08] pointer-events-none"
            style={{
              backgroundImage:
                "radial-gradient(1000px 700px at 80% -20%, rgba(255,255,255,.7), rgba(0,0,0,0))",
            }}
          />
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight">
                <Gilded>Delegate Registration • Noir MUN 2025</Gilded>
              </h1>
          ...
          <div className="mt-8 grid md:grid-cols-2 gap-6">
            ...
            {/* Right */}
            <div className="space-y-5">
              <NoirCard className="p-4 border-white/10 bg-white/[0.03]">
                <div className="text-sm font-semibold text-white/90">Committee Preferences</div>
                <div className="mt-3 grid gap-4">
                  ...
                  <Field label="Portfolio Preference 1">
                    <Input
                      ...
                      placeholder="Your top portfolio"
                    />
                  </Field>
                  <Field label="Portfolio Preference 2">
                    <Input
                      ...
                      placeholder="Alternate portfolio"
                    />
                  </Field>
                  ...
                  <Field label="Portfolio Preference 1 (Pref 2)">
                    <Input
                      ...
                      placeholder="Portfolio for 2nd choice"
                    />
                  </Field>
                  <Field label="Portfolio Preference 2 (Pref 2)">
                    <Input
                      ...
                      placeholder="Alternate for 2nd choice"
                    />
                  </Field>

                  {(f.committeePref1 === "IPL" || f.committeePref2 === "IPL") && (
                    <Field
                      label="Co-Delegate Details (IPL only)"
                      required
                      error={err.iplCoDelegate}
                      hint="Name, email, phone — required for double delegation"
                    >
                      <Textarea
                        value={f.iplCoDelegate}
                        onChange={(e) => update({ iplCoDelegate: e.target.value })}
                        placeholder="Enter co-delegate details"
                      />
                    </Field>
                  )}
                </div>
          ...
          {/* Payment */}
          <NoirCard className="mt-8 p-5">
            <div className="flex items-center gap-2 text-white/90 font-semibold">
              <Shield size={18} /> Payment & Proof
            </div>
            <p className="mt-2 text-sm text-white/70">
              Early Bird: <b>₹2,000</b> per delegate. IPL: <b>₹2,000</b> per delegate.
              Pay via UPI <span className="opacity-80">{upiPrimary}</span> or bank details given in form.
            </p>

            <div className="grid lg:grid-cols-[1fr_280px] gap-6 mt-4">
              {/* left: selectors + upload */}
                ...
                <Field label="Payment Option">
                  <Select
                    ...
                    options={[
                      `UPI: ${upiPrimary}`,
                      `UPI: ${upiAlt}`,
                      "Bank Transfer: JSFB0004049 (A/C 4049010060672314)",
                    ]}
                  />
                </Field>
              ...
              <div className="rounded-2xl border border-white/15 bg-white/[0.06] p-3">
                ...
                <div className="mt-3 text-[12px] text-white/60 text-center">
                  Tip: After paying, upload your screenshot here. We store it in Drive and your Sheet gets the link.
                </div>
              </div>
            </div>
          </NoirCard>
      ...
      {/* persistent, premium disclaimer bar */}
      ...
      <AnimatePresence>
        {!loggedIn && (
          <motion.div
            ...
            exit={{ y: 20, opacity: 0 }}
            className="fixed bottom-20 right-5 z-40 w-[calc(100%-2.5rem)] max-w-sm"
          >
            <div className="rounded-2xl border border-white/15 bg-white/[0.10] backdrop-blur p-3 text-sm text-white/90 shadow-xl">
              ...
              <div className="mt-3 flex gap-2">
                <Link
                  to="/login"
                  className="inline-flex items-center gap-1 rounded-xl px-3 py-1.5 bg-white/15 border border-white/20 hover:bg-white/25"
                >
                  Log in <ChevronRight size={14} />
                </Link>
                <Link
                  to="/signup"
                  className="inline-flex items-center gap-1 rounded-xl px-3 py-1.5 bg-white/10 border border-white/20 hover:bg-white/20"
                >
                  Sign up
                </Link>
              </div>
